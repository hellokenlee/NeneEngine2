...
namespace nene::g
{
	...
	void camera_component::on_notified(const event& e)
	{
		bool b_dirty = false;
		switch (e.m_id)
		{
		case event_id::keyboard_event:
			{
				...
				{
				case 'w':
					m_location += m_forward * m_move_speed;
					b_dirty = true;
					break;
				case 's':
					m_location -= m_forward * m_move_speed;
					...
					break;
				case 'a':
					m_location -= (m_right * m_move_speed);
					b_dirty = true;
					break;
				case 'd':
					m_location += (m_right * m_move_speed);
		...
		case event_id::mouse_event:
			{
				...
				if (!ee.m_delta_motion.equals(float2::zero()))
				{
		...
		if (b_dirty && m_render_view != nullptr)
		{
			update_render_view();
		}
	}

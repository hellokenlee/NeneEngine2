...
namespace r
{
	...
	{
		CHECK(m_gpi_shaders[static_cast<uint8>(stype)] == nullptr);
		...
		m_gpi_shaders[static_cast<uint8>(stype)] = gapi_dynamic::get().get_device()->create_and_compile_shader(stype, source,translator_input.m_entry, gapi_shader_feature_level::sm_6_0,  main_body_file_path);
	}

	const std::shared_ptr<i::gapi_shader>& shader_map::get_shader(gapi_shader_stage stype) const
	{
		return m_gpi_shaders[static_cast<uint8>(stype)];
	}
}

...
// different stage shader's registers MUST NOT overlap, we share a default register space  
// refs: https://www.3dgep.com/learning-directx-12-2/#shader-register-register-spaces
static constexpr auto NENE_D3D_DEFAULT_REGISTER_SPACE = 0;
static constexpr auto NENE_D3D_STATIC_SAMPLER_REGISTER_SPACE_SIZE = 1000;
...
namespace
{
	D3D12_STATIC_SAMPLER_DESC make_static_sampler(D3D12_FILTER d3d_filter, D3D12_TEXTURE_ADDRESS_MODE d3d_wrap_mode, uint32 shader_register)
	{
		D3D12_STATIC_SAMPLER_DESC d3d_desc = {
			...
			.RegisterSpace = NENE_D3D_STATIC_SAMPLER_REGISTER_SPACE_SIZE,
...
const std::array g_d3d12_static_sampler_descs = {
	...
	make_static_sampler(D3D12_FILTER_MIN_MAG_LINEAR_MIP_POINT, D3D12_TEXTURE_ADDRESS_MODE_WRAP, 2),
	make_static_sampler(D3D12_FILTER_MIN_MAG_LINEAR_MIP_POINT, D3D12_TEXTURE_ADDRESS_MODE_CLAMP, 3),
	make_static_sampler(D3D12_FILTER_MIN_MAG_MIP_LINEAR, D3D12_TEXTURE_ADDRESS_MODE_WRAP, 4),
	make_static_sampler(D3D12_FILTER_MIN_MAG_MIP_LINEAR, D3D12_TEXTURE_ADDRESS_MODE_CLAMP, 5),
};
...
const WinComPtr<ID3D12RootSignature>& d3d12_root_signature_manager::find_or_create_root_signature(ID3D12Device* d3d_device, const gapi_bound_shader_state_desc& bound_shader_state_desc)
{
	//
			...
			switch (root_parameter_type)
			{
				case D3D12_ROOT_PARAMETER_TYPE_32BIT_CONSTANTS:
				{
					...
					for (uint32 shader_register = 0; shader_register < shader_register_count.num_constant_buffer && shader_register < NUM_D3D_MAX_ROOT_CONSTANT_BUFFERS; ++shader_register)
					{
						...
						root_parameters[root_parameter_count++].InitAsConstantBufferView(shader_register, NENE_D3D_DEFAULT_REGISTER_SPACE, D3D12_ROOT_DESCRIPTOR_FLAG_DATA_STATIC, shader_visibility);
					}
					break;
				}
				case D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE:
				{
					...
					if (shader_register_count.num_constant_buffer >= NUM_D3D_MAX_CONSTANT_BUFFERS)
					{
						...
						descriptor_ranges[root_parameter_count].Init(
							...
							NENE_D3D_DEFAULT_REGISTER_SPACE,

...
namespace nene
{
	class gapi_d3d12_swap_chain : public gapi_swap_chain
	{
	public:
		...
		const std::shared_ptr<gapi_texture>& get_back_buffer() const override;

		const std::shared_ptr<gapi_texture>& get_back_buffer(size_t index) const override { return m_back_buffer_textures[index]; }
	...
	protected:
		...
		std::vector<std::shared_ptr<gapi_texture>> m_back_buffer_textures;
	};
}

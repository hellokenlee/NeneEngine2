...
{
	auto root_signature_mgr = device->get_root_signature_mgr();
	auto root_signature = root_signature_mgr->obtain_root_signature(args);

	D3D12_GRAPHICS_PIPELINE_STATE_DESC desc = args.m_desc;
	
	desc.pRootSignature = root_signature->get_d3d_root_signature();

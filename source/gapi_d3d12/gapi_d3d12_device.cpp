...
namespace nene
{
	...
	std::shared_ptr<gapi_cmd_allocator> gapi_d3d12_device::create_cmd_allocator(gapi_cmd_type cmd_type)
	{
	...
	std::shared_ptr<gapi_pipeline_state> gapi_d3d12_device::create_compute_pipeline_state(const gapi_compute_pipeline_state_desc& desc)
	{
		//
	...
	std::shared_ptr<gapi_pipeline_state> gapi_d3d12_device::create_graphics_pipeline_state(const gapi_graphics_pipeline_state_desc& desc)
	{
		//
	...
	std::shared_ptr<gapi_resource_view_allocator> gapi_d3d12_device::create_resource_view_allocator(gapi_resource_view_type view_type, gapi_resource_view_allocator_type allocator_type)
	{
		if (allocator_type == gapi_resource_view_allocator_type::online)
		{
			return std::make_shared<gapi_d3d12_online_resource_view_frame_allocator>(m_d3d_device, view_type); 
		}
		else
		{
			return std::make_shared<gapi_d3d12_offline_resource_view_page_allocator>(m_d3d_device, view_type);		
		}
	}

	void gapi_d3d12_device::create_constant_buffer_view(const std::shared_ptr<gapi_resource_view>& allocated_view, const std::shared_ptr<gapi_buffer>& buffer)
	{
		//
		CHECK(allocated_view->get_type() == gapi_resource_view_type::constant_buffer_view);
		//
		...
		desc.SizeInBytes = static_cast<uint32_t>(d3d_buffer->get_resource_desc().buffer_size());
		//
		m_d3d_device->CreateConstantBufferView(&desc, d3d_view.get_d3d_cpu_handle());
	}

	void gapi_d3d12_device::create_shader_resource_view(const std::shared_ptr<gapi_resource_view>& allocated_view, const std::shared_ptr<gapi_resource>& resource)
	{
		//
		CHECK(allocated_view->get_type() == gapi_resource_view_type::shader_resource_view);
		//
	...
	void gapi_d3d12_device::create_unordered_access_view(const std::shared_ptr<gapi_resource_view>& allocated_view, const std::shared_ptr<gapi_resource>& resource)
	{
		//
		CHECK(allocated_view->get_type() == gapi_resource_view_type::unordered_access_view);
		//
		const auto& d3d_resource = t::gapi_cast<gapi_d3d12_resource>(resource);
		auto& d3d_view = t::gapi_pin<gapi_d3d12_offline_resource_view>(allocated_view);
		//
	...
	void gapi_d3d12_device::create_render_target_view(const std::shared_ptr<gapi_resource_view>& allocated_view, const std::shared_ptr<gapi_texture>& texture)
	{
		//
		CHECK(allocated_view->get_type() == gapi_resource_view_type::render_target_view);
		//
	...
	void gapi_d3d12_device::create_depth_stencil_view(const std::shared_ptr<gapi_resource_view>& allocated_view, const std::shared_ptr<gapi_texture>& texture)
	{
		//
		CHECK(allocated_view->get_type() == gapi_resource_view_type::depth_stencil_view);
		//
		const auto& d3d_resource = t::gapi_cast<gapi_d3d12_texture>(texture);
		auto& d3d_view = t::gapi_pin<gapi_d3d12_offline_resource_view>(allocated_view);
		//
	...
	void gapi_d3d12_device::create_sampler(const std::shared_ptr<gapi_resource_view>& allocated_view, const gapi_sampler_desc& desc)
	{
		//
		CHECK(allocated_view->get_type() == gapi_resource_view_type::texture_sampler);
		//
	...
	void gapi_d3d12_device::copy_resource_view(const std::shared_ptr<gapi_resource_view>& dst_view, const std::shared_ptr<gapi_resource_view>& src_view)
	{
		CHECK(dst_view->get_type() == src_view->get_type());
		const auto& d3d_dst_view = t::gapi_pin<gapi_d3d12_offline_resource_view>(dst_view);
		const auto& d3d_src_view = t::gapi_pin<gapi_d3d12_offline_resource_view>(src_view); 
		m_d3d_device->CopyDescriptorsSimple(1, d3d_dst_view.get_d3d_cpu_handle(), d3d_src_view.get_d3d_cpu_handle(), d3d_cast(dst_view->get_type()));
	}

	std::shared_ptr<gapi_resource_allocator> gapi_d3d12_device::create_resource_allocator()
	{
		NOT_IMPLEMENTED();
		return nullptr;
	}

	std::shared_ptr<gapi_resource> gapi_d3d12_device::create_resource(const gapi_resource_desc& desc)
	{
		...
		// constant buffer must be aligned
		if (desc.is_buffer() && t::has_flag(desc.m_buffer_usage_flag, gapi_buffer_usage_flag::constant_buffer))
		{
	...
	std::shared_ptr<gapi_resource> gapi_d3d12_device::create_placed_resource(const gapi_resource_desc& desc)
	{
		NOT_IMPLEMENTED();
		return nullptr;
	}

	std::shared_ptr<gapi_resource> gapi_d3d12_device::create_reserved_resource(const gapi_resource_desc& desc)
	{
		NOT_IMPLEMENTED();
		return nullptr;
	}

	std::vector<std::shared_ptr<gapi_buffer_sublayout>> gapi_d3d12_device::calculate_buffer_layout(const std::shared_ptr<gapi_resource>& dst_resource, uint32_t dst_start_subindex, uint32_t num_subresources, uint64_t& out_num_total_bytes)
	{
		const auto& d3d_resource = t::gapi_pin<gapi_d3d12_resource>(dst_resource);
		// @KenLee: Why microsoft dont return a `const D3D12_RESOURCE_DESC*` in msvc?
		auto desc = d3d_resource.get_d3d_resource()->GetDesc();
		//
		std::vector<D3D12_PLACED_SUBRESOURCE_FOOTPRINT> footprints(num_subresources);
		std::vector<uint32_t> num_rows(num_subresources);
		std::vector<uint64_t> row_size_in_bytes(num_subresources);
		//
		m_d3d_device->GetCopyableFootprints(&desc, dst_start_subindex, num_subresources, 0, footprints.data(), num_rows.data(), row_size_in_bytes.data(), &out_num_total_bytes);
		//
		std::vector<std::shared_ptr<gapi_buffer_sublayout>> result;
		for (uint32_t i = 0; i < num_subresources; ++i)
		{
			result.emplace_back(std::make_shared<gapi_d3d12_buffer_sublayout>(footprints[i], num_rows[i], row_size_in_bytes[i]));
		}
		return result;
	}

	std::shared_ptr<gapi_shader> gapi_d3d12_device::create_and_compile_shader(const gapi_shader_stage& stype, const std::string& source, const std::string& entry, const gapi_shader_feature_level& level, const std::string& debug_name)
	{

...
gapi_d3d12_resource_view_allocator::gapi_d3d12_resource_view_allocator(const WinComPtr<ID3D12DescriptorHeap>& heap, const uint32& num_descriptors, const uint32& descriptor_size)
	: super()
...
std::shared_ptr<i::gapi_resource_view> gapi_d3d12_resource_view_allocator::allocate_resource_view()
{
	...
	return std::make_shared<gapi_d3d12_resource_view>(index, handle);
}

void gapi_d3d12_resource_view_allocator::free_resource_view(std::shared_ptr<i::gapi_resource_view>& view)
{
	const auto& d3d_view = gapi_d3d12_resource_view::cast(view);
	uint32 index;

...
public:
	...
	void clear_depth_stencil_view(const std::shared_ptr<i::gapi_resource_view>& depth_stencil, const float& depth, const uint8_t& stencil) override;
	...
	void copy_buffer_region(const std::shared_ptr<i::gapi_buffer>& dst, uint32_t dst_offset, const std::shared_ptr<i::gapi_buffer>& src, uint32_t src_offset, uint64_t num_bytes) override;
	void copy_buffer_region(const std::shared_ptr<i::gapi_texture>& dst, uint32_t dst_subindex, const std::shared_ptr<i::gapi_buffer>& src, const std::shared_ptr<i::gapi_buffer_sublayout>& src_sublayout) override;
	void copy_texture_region(const std::shared_ptr<i::gapi_texture>& dst, uint32_t dst_subindex, const std::shared_ptr<i::gapi_texture>& src, uint32_t src_subindex) override;
	...
	void execute_indirect(const std::shared_ptr<i::gapi_cmd_layout>& layout, uint32_t max_num_cmd, const std::shared_ptr<i::gapi_buffer>& arg_buffer, uint32_t arg_buffer_offset, const std::shared_ptr<i::gapi_buffer>& count_buffer, uint32_t count_buffer_offset) override;

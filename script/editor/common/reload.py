...
def _function_reloader(oldfunc, newfunc, depth):
    b_dirty = False
    if oldfunc.__code__.co_code != newfunc.__code__.co_code or oldfunc.__code__.co_consts != newfunc.__code__.co_consts:
        setattr(oldfunc, "__code__", newfunc.__code__)
        b_dirty = True
    if oldfunc.__defaults__ != newfunc.__defaults__:
        setattr(oldfunc, "__defaults__", newfunc.__defaults__)
        b_dirty = True
    if oldfunc.__doc__ != newfunc.__doc__:
...
def find_reloader(oldobj):
    import inspect
    if hasattr(oldobj, "__module__"):
        for prefix in _white_list_module_prefix:
            if oldobj.__module__ is not None and oldobj.__module__.startswith(prefix):
                return None
    if inspect.isclass(oldobj):
        return _class_reloader
    elif inspect.ismethod(oldobj):
        return _method_reloader
    elif inspect.isfunction(oldobj):
        return _function_reloader
    return None
